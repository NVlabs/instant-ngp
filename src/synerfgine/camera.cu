...
Camera::Camera(const vec3& eye, const vec3& at, const vec3& up) : 
    m_camera(mat4::identity()), m_rotate(mat4::identity()), 
...
void Camera::set_look_at(const vec3& pos) {
    auto full = m_eye - pos;
    m_rotate[0] = vec4(normalize(cross(m_up, full)), 0.0);
    m_rotate[1] = vec4(normalize(cross(full, m_camera[0].xyz())), 0.0);
    m_rotate[2] = vec4(normalize(full));
    m_rotate[3] = vec4(vec3(0.0), 1.0);
